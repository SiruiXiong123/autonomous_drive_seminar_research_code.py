...
def compute_ellipsoid_risk(vehicles_info, a=4, b=2, px=2, py=3, mf=0.5, pt=1,v_s = 2):
    risk_list = []
    # 假设 vehicles_info 已经按离本车距离从近到远排序
    for idx, (dx, dy, dvx, dvy) in enumerate(vehicles_info):
        if idx == 0:
            effective_a = a + 5 + v_s
            effective_b = b + 0.05 * v_s
        else:
            effective_a = a
            effective_b = b

        # 注意：确保公式中各部分运算优先级正确
        # 例如，假设风险函数为：Ec = mf / ( (abs(dx)/effective_a)**px + (abs(dy)/b)**py + 1 )**pt
        Ec = mf / (((abs(dx) / effective_a) ** px + (abs(dy) / b) ** py + 1) ** pt)

        # 对于特殊的无效车辆信息，可以设为 0 风险
        ...
        # 可选择打印最近车辆的信息
        # if idx == 0:
            # print("最近的车辆参数:", (dx, dy, dvx, dvy))
            # print("最近车辆风险:", cfj, "mf:", mf)
...
class MetaDriveEnv(BaseEnv):
    ...
    def reward_function(self, vehicle_id: str):
        ...
        safe_a =  15
        safe_b =  5
        total_risk = compute_ellipsoid_risk(vehicles_info,a=safe_a,b=safe_b,v_s=v_s)
        reward -=  0.05 * total_risk
        ...
        if delta > 0 and vehicle.speed_km_h > 30:
