...
class MetaDriveEnv(BaseEnv):
    ...
    def reward_function(self, vehicle_id: str):
        ...
        heading_chk = vehicle.navigation._navi_info[0]
        rhs_chk = vehicle.navigation._navi_info[1]
        dist_to_chk = 0.0  # 0到1之间，0小车拿到check point,1未拿到
        chk_reward = 0.0
        bendradius = vehicle.navigation._navi_info[2]  # vehicle.navigation._navi_info[2] 0弯，1直

        # if True:  # 如果前方无车 (安全条件)
        #     dist_to_chk = math.sqrt(heading_chk ** 2 + rhs_chk ** 2)
        # else:
        #     dist_to_chk = 1
        #
        # if dist_to_chk == 0 :
        ...
        # reward += 0.02 * lateral_factor
        # reward -= 0.08 * (1-lateral_factor)# 只要超车的奖励比不维持平衡的惩罚高就行
        # reward += self.config["checkpoint_reward"] * (0.2 * (1 - dist_to_chk) + chk_reward ) #+ 0.01 * (1-rhs_chk)
        reward -= 0.1 * (1-heading_diff)
        reward -= 0.06 * (steer_diff)
        reward += 0.04 * (1-steer_diff)
        # reward -= 0.01 * danger_efficient
        # reward += 0.1 * overtake_reward
